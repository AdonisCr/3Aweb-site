      ...
      <USection>
        <div className="space-y-12">
          {articles.map((article, i) => (
            <div
              key={i}
              className="flex items-start gap-8 pb-12 border-b border-gray-100 last:border-0"
            >
              <div className="rounded-card overflow-hidden flex-shrink-0" style={{ width: 380, height: 250 }}>
                <img src={article.image} alt={article.title} className="w-full h-full object-cover" />
              </div>
              <div className="flex-1">
                <h2 className="mb-2 text-heading-md text-dark">{article.title}</h2>
                <p className="mb-4 text-body-sm font-bold text-dark">Publié le {article.date}</p>
                <p className="mb-6 text-body-md text-body">{article.excerpt}</p>
                <UButton to="#" variant="text">
                  Lire l'article
